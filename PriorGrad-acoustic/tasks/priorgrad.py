...
def g2p(txt):
    ...
    txt = re.sub(r" +", r" ", re.sub(f"[^{''.join(characters)}]", r" ", txt.upper())).strip()
    txt = ' '.join(list(txt))

    txt = re.sub(r"[OG]{1} '", lambda x: x.group().replace(" '", "1"), txt)
    txt = re.sub(r"[CS]{1} H", lambda x: x.group().replace(" H", "1"), txt)
    txt = re.sub(r"(N G [^AEIOU]{1})|(N G$)", lambda x: 'N'+x.group()[2:], txt)
    return txt.replace('   ', ' | ')
